...
def getKpis(list_id):
    ...
    response = requests.get(url, headers=headers, params="") #response = requests.get(url, headers=headers, params=query)
    if response.status_code != 200:
        return jsonify({"error": "Failed to fetch data from ClickUp API"}), response.status_code
